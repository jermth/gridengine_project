...
def get_sge_job_details():
    if not _has_jobs():
        return {}

    # Query parse and return job details
    qstat = subprocess.Popen('. /etc/cluster-setup.sh && qstat -u "*" -j "*" -xml', shell=True, stdout=subprocess.PIPE)
    job_details = _parse_job_details(qstat.stdout)
    return job_details
...
def get_sge_jobs():
    ...
    qstat = subprocess.Popen('. /etc/cluster-setup.sh && qstat -g d -xml -u "*"', shell=True, stdout=subprocess.PIPE)
    jobs = _parse_jobs(qstat.stdout)
    return jobs

def get_host_complexes(complexes=None):
    """ Get a dict of hostnames to complex values 

    Example: complexes=['slot_type', 'affinity_group']

    {
        "ip-10-10-10-10": {'slot_type': 'execute', 'affinity_group': 'group1'},
        "ip-11-11-11-11": {'slot_type': 'execute', 'affinity_group': 'group2'}
    }
    """
