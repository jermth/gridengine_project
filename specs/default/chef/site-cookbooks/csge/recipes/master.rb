...
bash "clear old hosts" do
  ...
  done && touch #{chefstate}/csge.clear.hosts
  EOH
  creates "#{chefstate}/csge.clear.hosts"
  action :run
end
...
execute "setglobal" do
  command ". /etc/cluster-setup.sh && qconf -Mconf #{sgeroot}/conf/global && touch #{chefstate}/csge.global.set"
  creates "#{chefstate}/csge.global.set"
  action :run
end

execute "setsched" do
  command ". /etc/cluster-setup.sh && qconf -Msconf #{sgeroot}/conf/sched && touch #{chefstate}/csge.sched.set"
  creates "#{chefstate}/csge.sched.set"
  action :run
end
...
execute "schedexecinst" do
  command "cd #{sgeroot} && ./inst_sge -x -noremote -auto #{sgeroot}/conf/#{nodename}.conf && touch #{chefstate}/csge.sgesched.schedexecinst"
  creates "#{chefstate}/csge.sgesched.schedexecinst"
  action :run
end
...
template "#{sgeroot}/conf/csge.q" do
  source "csge.q.erb"
  owner "root"
  group "root"
  mode "0755"
...
execute "set complexes" do
  command ". /etc/cluster-setup.sh && qconf -Mc #{sgeroot}/conf/complexes && touch #{chefstate}/csge.setcomplexes.done"
  creates "#{chefstate}/csge.setcomplexes.done"
  action :run
end
...
#
execute "csge.qcfg" do
  command ". /etc/cluster-setup.sh && qconf -Rattr queue #{sgeroot}/conf/csge.q all.q && touch #{chefstate}/csge.qcfg"
  only_if "test -f #{sgeroot}/SGESuspend.sh && test -f #{sgeroot}/SGETerm.sh && test -f #{sgeroot}/conf/csge.q && test ! -f #{chefstate}/csge.qcfg"
end
